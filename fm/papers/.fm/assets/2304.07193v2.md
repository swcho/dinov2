...
<span id="page-1-0"></span>![](2304.07193v2/_page_1_Picture_1.jpeg)
...
<span id="page-2-0"></span>![](2304.07193v2/_page_2_Figure_1.jpeg)

Figure 2: **Evolution of performance when scaling in parameters.** We show performance on eight types of vision tasks, as presented in Sec. [7,](#page-10-0) and average metrics with each type. Features are extracted from our self-supervised encoders, DINOv2 (dark blue), and we compare them with self-supervised methods (pale orange), as well as weakly-supervised methods (dark pink). We report the best-performing weaklysupervised model's performance as a dashed horizontal line. Our family of models drastically improves over the previous state of the art in self-supervised learning and reaches performance comparable with weaklysupervised features. See Sec. [7](#page-10-0) for a detailed analysis.

Finally, we provide a variety of pretrained visual models, called DINOv2, trained with different Vision Transformers (ViT) [\(Dosovitskiy et al., 2016\)](#page-22-3) architectures on our data. We release all the models and the code to retrain DINOv2 on any data. We validate the quality of DINOv2 on various computer vision benchmarks at both image and pixel levels as we scale them, as summarized in Fig. [2.](#page-2-0) We conclude that selfsupervised pretraining alone is a good candidate for learning transferable frozen features that are competitive with the best openly available weakly-supervised models.

## **2 Related Work**

**Intra-image self-supervised training.** A first family of self-supervised methods focuses on pretext tasks built from the image, i.e., extracting a signal from the image to be predicted from the rest of the image. This idea has become prevalent with the work of [Doersch et al.](#page-22-4) [\(2015\)](#page-22-4), where they train by predicting the context of a given patch. Many other pretext tasks were introduced based on, for example, re-colorizing images [\(Zhang et al., 2016\)](#page-27-2), predicting transformations [\(Gidaris et al., 2018\)](#page-23-4), inpainting [\(Pathak et al.,](#page-25-5) [2016\)](#page-25-5) or patch re-ordering [\(Noroozi & Favaro, 2016;](#page-25-6) [Misra & Maaten, 2020\)](#page-25-7). Recently, the emergence of patch-based architectures, like ViTs, has led to a revisit of inpainting for pre-training [\(He et al., 2022;](#page-23-0) [Bao](#page-21-6) [et al., 2021;](#page-21-6) [El-Nouby et al., 2021\)](#page-22-5), potentially in feature space [\(Assran et al., 2023;](#page-21-7) [Baevski et al., 2022\)](#page-21-8). Of particular interest, [He et al.](#page-23-0) [\(2022\)](#page-23-0) show that a masked auto-encoder (MAE) learns features that provide substantial improvements when finetuned on downstream tasks. This property of MAEs has been further validated on video [\(Tong et al., 2022\)](#page-26-3), audio [\(Xu et al., 2022\)](#page-27-3), and across other modalities [\(Girdhar et al.,](#page-23-5) [2023\)](#page-23-5). However, their features require supervised finetuning, while our features perform well out of the box.

**Discriminative self-supervised learning.** The second line of work, closer to ours, is using discriminative signals between images or groups of images to learn features. This family of methods has roots in early deep learning work [\(Hadsell et al., 2006\)](#page-23-6) but became popular with the emergence of instance classification methods [\(Dosovitskiy et al., 2016;](#page-22-3) [Bojanowski & Joulin, 2017;](#page-21-9) [Wu et al., 2018\)](#page-27-4). Several improvements

<span id="page-3-0"></span>![](2304.07193v2/_page_3_Diagram_1.jpeg)
...
where  $d_{n,i} = \min_{j \neq i} \|x_i - x_j\|$  is the minimum distance between  $x_i$  and any other point within the batch. We also  $\ell_2$ -normalize the features before computing this regularizer.
...
<span id="page-8-0"></span>![](2304.07193v2/_page_8_Figure_1.jpeg)
...
<span id="page-9-0"></span>![](2304.07193v2/_page_9_Figure_1.jpeg)
...
<span id="page-9-1"></span>![](2304.07193v2/_page_9_Figure_3.jpeg)
...
| Method CLIP | Arch. ViT-L/14 | Data Weakly WIT-400M | Text sup. supervised ✓ | kNN val 79.8 | val 84.3 | linear ReaL 88.1 | V2 75.3 |
|-------------|----------------|----------------------|------------------------|--------------|----------|------------------|---------|
...
| EVA-CLIP    | ViT-g/14       | custom ∗             | ✓ Self-supervised      | 83.5         | 86.4     | 89.3             | 77.4    |
...
| Feature  | Arch     | iNat2018 | Image classification iNat2021 | Places205 | K400 | Video UCF-101 | classification SSv2 |
|----------|----------|----------|-------------------------------|-----------|------|---------------|---------------------|
...
| Feature  | Arch     | M    | Oxford H | M    | Paris H | GAP  | Met GAP- | ACC  | AmsterTime mAP |
|----------|----------|------|----------|------|---------|------|----------|------|----------------|
...
| Method   | Arch.    | lin. | ADE20k (62.9) +ms | lin. | CityScapes (86.9) +ms | Pascal lin. | VOC (89.0) +ms |
|----------|----------|------|-------------------|------|-----------------------|-------------|----------------|
...
| Method   | Arch.    | lin.  | NYUd (0.330) 1 lin. | 4 DPT | lin. | KITTI (2.10) 1 lin. | 4 DPT | NYUd lin. | → SUN (0.421) 1 lin. | RGB-D 4 DPT |
|----------|----------|-------|---------------------|-------|------|---------------------|-------|-----------|----------------------|-------------|
...
<span id="page-16-0"></span>![](2304.07193v2/_page_16_Picture_1.jpeg)
...
<span id="page-16-1"></span>![](2304.07193v2/_page_16_Picture_3.jpeg)
...
<span id="page-17-0"></span>![](2304.07193v2/_page_17_Picture_1.jpeg)
...
<span id="page-18-0"></span>![](2304.07193v2/_page_18_Picture_1.jpeg)
...
| SEERv2 | RT-10B   | IR-1B    | 59.7           | 78.5   | 86.6 | 65.9   | 76.3    | 81.1     | 85.6   |
...
| Model    | Assoc.         | female darker | Gender female lighter | Skintone male darker | male lighter | 18-30 | Age 30-45 | Groups 45-70 | 70+  |
|----------|----------------|---------------|-----------------------|----------------------|--------------|-------|-----------|--------------|------|
...
| Model to Reproduce | GPU Type  | GPU Power consumption | GPU-hours | PUE | Total power consumption | Carbon emitted (tCO <sub>2</sub> eq) |
|--------------------|-----------|-----------------------|-----------|-----|-------------------------|--------------------------------------|
